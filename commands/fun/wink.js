...
if (args[0]=== "me"||args[0]=== `<@!${message.author.id}>`) {
      ...
      userm = await message.guild.members.fetch(args[0].replace("<@!", "").replace("<@","").replace(">","")).catch(err => { return message.channel.send("<:tairitsuno:801419553933492245> | Remember to mention a valid user to wink!") })
      usern =userm.user
      //userm =use.user
