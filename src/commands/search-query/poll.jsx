...
const sendEmbed= await message.channel.send({embeds:[pollEmbed]})
  await sendEmbed.react("👍")
  await sendEmbed.react("👎")
 const reactionx = (reaction) => reaction.emoji.name ==='👎'
 const reactionv = (reaction) => reaction.emoji.name === "👍" //, {time: time});
 const collectorv = sendEmbed.createReactionCollector({filter:reactionv, time})
 
 
 const collectorx = sendEmbed.createReactionCollector({filter:reactionx, time})
...
👎(No): ${collectx.size}`)
     console.log("ok!")
  sendEmbed.edit({embeds:[CompleteEmbed]})
    })
  })
