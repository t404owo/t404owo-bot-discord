...
serverQueue.songs.forEach((s)=>{
 data.push(s) 
    ...
    if(serverQueue.songs.length>11&&parseInt(page)+1 <= Math.ceil(Object.keys(serverQueue.songs).length / 10))queue.setFooter({text:`Type ${client.config.prefix}queue ${parseInt(page)+1} or /queue page:${parseInt(page)+1} to see page ${parseInt(page)+1}.`})
    ...
    console.log("it should reply")
    message.reply({embeds:[queue], allowedMentions: { repliedUser: false }})
  },
  options:[
    {
    name: "page",
  ...
  interaction: async function (client, message, arg) {
    ...
    if(arg&&parseInt(args[0])> Math.ceil(Object.keys(serverQueue.songs).length / 10)) return sendError(`<:tairitsuno:801419553933492245> | The queue currently only has \`${Math.ceil(Object.keys(serverQueue.songs).length / 10)}\` pages!`, message,)
    if(arg&&parseInt(args[0])<2)return sendError(`<:tairitsuno:801419553933492245> | Please give a number that is higher than **1**!`, message)
    }
    catch(e){
      args=['1']
    }
...
serverQueue.songs.forEach((s)=>{
 data.push(s) 
    ...
    .addField("Text Channel", `<#${serverQueue.textChannel.id}>`)
    .addField("Voice Channel", `<#${serverQueue.voiceChannel.id}>`)
    if(serverQueue.songs.length>11&&parseInt(page)+1 <= Math.ceil(Object.keys(serverQueue.songs).length / 10))embed.setFooter({text:`Type ${client.config.prefix}queue ${parseInt(page)+1} or /queue page:${parseInt(page)+1} to see page ${parseInt(page)+1}.`})
