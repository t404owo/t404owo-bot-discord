module.exports.run = async (bot, message, args) => {
  ...
  if (!perm.has(Permissions.FLAGS.MANAGE_GUILD) && !perm.has(Permissions.FLAGS.ADMINISTRATOR))
  ...
  if (args[0] === "me" || args[0] === `<@!${message.author.id}>`) {
    ...
    if (args[0]) message.xpadd = args[1];
    else message.xpadd = args[0];
  } else if (args[0]) {
    if (!message.guild) return;

    userm = await message.guild.members
      .fetch(args[0].replace("<@!", "").replace("<@", "").replace(">", ""))
      .catch((err) => {
        userm = message.author;
        message.mention = message.author;
        ...
        if (level > lvl) {
          ...
          message.channel
            .send({
              content: `<@!${message.mention.id}> is now on level ${newLevel} in this server!`,
            })
            .then((ms) => {
