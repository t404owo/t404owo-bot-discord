  ...
  message.reply({
    embeds: [
      embed
        ...
        .setTitle(
          `__${bot.db.get(
            `${message.guild.id}npcname_${a.toLowerCase()}`
          )}__'s info`
        )
        .setDescription(
          `\n${
            bot.db.get(`${message.guild.id}npcdesc_${a.toLowerCase()}`) ||
            "No Description"
          }`
        )
        .addField(
          "Name",
          `${bot.db.get(`${message.guild.id}npcname_${a.toLowerCase()}`)}`
        )
        .addField(
          "Avatar",
          "[Click Here]" +
            `(${bot.db.get(`${message.guild.id}npcav_${a.toLowerCase()}`)})`
        )
        .setThumbnail(
          `${bot.db.get(`${message.guild.id}npcav_${a.toLowerCase()}`)}`
        ),
...
module.exports.info = {
  ...
  usage: "<npc_name>",
  aliases: ["tupperinfo", "npci", "tupperi"],
};
