...
exports.run = async (bot, message, args) => {
  ...
  if(!permissions.has(Permissions.FLAGS.KICK_MEMBERS)) return message.noMentionReply(`${process.env.EMOTE_NO || '<:tairitsuno:801419553933492245>'}`+" | I don't have permission to kick!!!");
 if (!perm.has(Permissions.FLAGS.KICK_MEMBERS)&&!perm.has(Permissions.FLAGS.MANAGE_GUILD)&&!perm.has(Permissions.FLAGS.MANAGE_MEMBERS)&&!perm.has(Permissions.FLAGS.ADMINISTRATOR))
   ...
   let BotRole = message.guild.members.cache.get(message.client.user.id).roles.highest.position;
    ...
    if (UserRole <= Role) return message.mentionReply(`${process.env.EMOTE_NO || '<:tairitsuno:801419553933492245>'}`+' | You can\'t kick that user because that user has a role position which is higher than yours, or has a same role position as you!');
      ...
      .then(msg => {
        ...
        if(!reasonb){
        msg.edit({content:`${process.env.EMOTE_OK || '<:hikariok:801419553841741904>'} | Kicked sucessfully`})
        };
      if(reasonb) {
        msg.edit({content:`${process.env.EMOTE_OK || '<:hikariok:801419553841741904>'} | Kicked sucessfully **|** ${reason}`});}
    });
