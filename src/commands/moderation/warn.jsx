...
exports.run = async(bot, message, args) => {
  ...
  if(!permissions.has(Permissions.FLAGS.MANAGE_MEMBERS)) return message.mentionReply(`${process.env.EMOTE_NO || '<:tairitsuno:801419553933492245>'}`+" | You don't have permission to ban!!!");
const perm=message.channel.permissionsFor(message.member)//perm.has()
  if(!perm.has(Permissions.FLAGS.MANAGE_MEMBERS)&&!perm.has(Permissions.FLAGS.MANAGE_GUILD)&&!perm.has(Permissions.FLAGS.ADMINISTRATOR)) return message.mentionReply(`${process.env.EMOTE_NO || '<:tairitsuno:801419553933492245>'}`+" | You don't have permission to warn!!!");
 ...
 let pUser = message.channel.permissionsFor(wUser)//perm.has()
  if(pUser.has(Permissions.FLAGS.ADMINISTRATOR)) return message.mentionReply(`${process.env.EMOTE_NO || '<:tairitsuno:801419553933492245>'}`+" | You don't have permission to warn an admin/mod!!!");
  ...
  if(!muterale) {
              try{
                ...
                message.guild.channels.cache.forEach(async (channel, id) => {
                  await channel.permissionOverwrite.create(message.guild.roles.cache.find(r => r.name === 'muted'), {
